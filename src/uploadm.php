...
<div class="container bg-light w-50" style="border-radius: 20px;min-height: 700px">
    ...
    <div class="container w-25" style="margin: 0px auto;">
        <img class="rounded img-fluid w-100" src="<?php
        if (isset($_GET['id']))
            echo IMAGE_ROOT . $img['PATH'];
        ?>" id="show" alt="图片未上传">
    ...
    <form action="modify.php" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <?php if (isset($_GET['id'])) {
                echo "<input type=\"text\" class=\"form-control\" id=\"img-tit\" required name=\"tit\" value=\"".$img['Title']."\">";
            } else echo '<input type="text" class="form-control" id="img-tit" required name="tit" value="">';
            ?>
        </div>
        <div class="form-group">
            <label for="img-des">图片描述:</label>
                <?php if (isset($_GET['id'])) {
                    echo "<textarea class=\"form-control\" rows=\"3\" id=\"img-des\" required name=\"des\">".$img['Description']."</textarea>";
                } else echo '<textarea class="form-control" rows="3" id="img-des" required name="des"></textarea>';
                ?>
        </div>
        <div class="form-group">
            ...
            <select class="form-control" id="slt-thm" name="thm">
                <?php
                if (isset($_GET['id'])) {
                    echo "<option value=\"" . $img['Content'] . "\"selected> " . $img['Content'] . "</option>";
                } else echo "<option value=\"\" selected disabled></option>";
                ?>
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="ctr" id="slt-ctr" onchange="setSelectCity()">
                <?php
                if (isset($_GET['id']))
                    echo '<option value="' . $img['CountryCodeISO'] . '" selected>' . $img['CountryCodeISO'] . '</option>';
                else echo '<option value="" selected disabled></option>';
                try {
                    $pdo = new PDO(DBCONNSTRING, DBUSER, DBPASS);
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="slt-cty" id="slt-cty"
                ...
                <?php
                if (isset($_GET['id']))
                    echo '<option value="' . $img['CityCode'] . '" selected>' . $img['CityCode'] . '</option>';
                else echo '<option value="" selected disabled></option>';
                ?>
            </select>
        </div>
        <br>
        <input type="text" style="display: none" name="upload" value="<?php echo $img['ImageID']; ?>">
        <button type="submit" class="btn btn-primary btn-block"
                onclick="let slt = document.getElementById('slt-cty');
                    let index = slt.selectedIndex;
                    document.getElementById('upload-cty').value = slt.options[index].text;">修改
        </button>
        <br>
    </form>
</div>
